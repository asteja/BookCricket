...
class ScoreViewController: UIViewController {
    ...
    @IBAction func scoreButtonClicked(_ sender: Any) {
         ...
         if (isLabelViewShowing) {
            //hide Graph
            UIView.transition(from: ballIV,to: labelVW,duration: 1.0,options: [UIViewAnimationOptions.transitionFlipFromLeft, UIViewAnimationOptions.showHideTransitionViews],completion:nil)
            displayScore()
        } else {
            //show Graph
            UIView.transition(from: labelVW,to: ballIV,duration: 1.0,options: [UIViewAnimationOptions.transitionFlipFromRight, UIViewAnimationOptions.showHideTransitionViews],completion: nil)
        }
